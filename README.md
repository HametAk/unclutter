...
- **On page visit**: analyze new templates in visible tabs, after a short render-settling delay. This automatically sends candidate snippets to Vercel / TypeSafe AI and incurs API charges. Off by default.
...
- Only extension background code calls the fixed Gateway endpoint. Popup-origin checks protect settings/manual analysis. Page-visit requests are validated and require the user's saved automatic-mode opt-in.
